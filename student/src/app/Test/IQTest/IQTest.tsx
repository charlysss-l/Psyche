...
const IQTest: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const storedTime = localStorage.getItem('iqTestTimer');
        if (storedTime) {
        ...
        const timerID = setInterval(() => {
            setTimer((prevTime) => {
                if (prevTime > 0) {
                    ...
                    localStorage.setItem('iqTestTimer', newTime.toString());
                    return newTime;
                } else {
    ...
    return (
        <form onSubmit={handleSubmit} className={style.formTest}>
            <h1>IQ Test</h1>
            <p className={style.numQuestion}>Number of Questions: {iqTest?.numOfQuestions}</p>
