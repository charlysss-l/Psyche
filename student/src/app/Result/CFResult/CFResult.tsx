...
import styles from './CFResult.module.scss';
...
const CFResult: React.FC = () => {
    const navigate = useNavigate();
    const [result, setResult] = useState<CFTestResultData | null>(null);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        const dataContent = response.data.find((content: any) => content.key === 'data_privacyCF');
        const outroContent = response.data.find((content: any) => content.key === 'outroCF');
...
const formatDate = (dateString: string) => {
    ...
    useEffect(() => {
        const storedResults = localStorage.getItem('cfTestResults');
        if (storedResults) {
            const parsedResults: CFTestResultData = JSON.parse(storedResults);
            setResult(parsedResults);

            // Fetch the CF test data based on the test ID to retrieve interpretations
            fetch(`${backendUrl}/api/CFtest/67a09ef7e3fdfebbf170a124`)
    ...
    const generatePDF = () => {
        ...
        pdf.text("Culture Fair Test", 105, 20, { align: "center" });
    
        pdf.setFontSize(16);
        pdf.text("( CF Test Individual Record Form )", 105, 30, { align: "center" });
        ...
        img.onload = () => {
            ...
            pdf.save("CFResult.pdf"); // Save the PDF after the logo is loaded
        };
    };
        ...
        <div className={styles.container} id="result-container">
            {result ? (
                <div>
                    ...
                    <h3 className={styles.subheading}>CF Test Result</h3>
