...
interface IConsultationRequest extends Document {
  ...
  date: Date;
  status: 'pending' | 'accepted' | 'declined' | 'cancelled' | 'completed' | 'deleted' | 'removed';
  message: string;
  firstName?: string;
  lastName?: string;
  sex?: string;
...
const ConsultationRequestSchema: Schema = new Schema({
  ...
  status: {
    type: String,
    default: 'pending',
    enum: ['pending', 'accepted', 'declined', 'cancelled', 'completed', 'deleted' , 'removed'], // Status can only be one of these
  },
  message: {
    type: String,
    required: true,
