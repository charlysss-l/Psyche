...
import style from './OmrCFResult.module.scss';
...
const OmrCFResult: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await axios.post(`${backendUrl}/api/omrcf`, dataToSubmit);
            alert('Test submitted successfully!');
            localStorage.setItem('cfTestResults', JSON.stringify(dataToSubmit));
            navigate('/cfresults_list_both');
        } catch (error) {
    ...
    return (
        <form onSubmit={handleSubmit} className={style.formTest}>
          <h1 className={style.omrcftitle}>OMR CF Test Result</h1>
