...
import styles from './CFStatistics.module.scss'; // Import your CSS module
...
const CFStatistics: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const onlineResponse = await fetch(`${backendUrl}/api/usercf`);
      if (!onlineResponse.ok) {
        throw new Error(`Network response was not ok: ${onlineResponse.statusText}`);
      }
      const onlineData = await onlineResponse.json();
  
      const physicalResponse = await fetch(`${backendUrl}/api/omrcf`);
      ...
      const combinedData: UserCFTest[] = [...onlineData.data, ...physicalData.data].map((result) => {
...
const filteredResults = results.filter((result) => {
  ...
  return (
    <div className={styles.reportContainer}>
      ...
      <h2 className={styles.heading}>CF Test Data</h2>
        ...
        <tbody>
          {filteredResults.map((result) => (
            <tr className={styles.userList} key={result.userID}>
              <td className={styles.idCflist}>{result.userID}</td>
            </tr>
          ))}
        </tbody>
        </div>
